...
REM === PYENV + VENV MODE (Injected by ChatGPT) ==================================
REM Configure to use pyenv-win Python 3.12.10 and an isolated venv, bypassing Conda.

REM Resolve script directory
set "SCRIPT_DIR=%~dp0"
if "%SCRIPT_DIR:~-1%"=="\" set "SCRIPT_DIR=%SCRIPT_DIR:~0,-1%"

REM pyenv-win root (default)
if not defined PYENV_ROOT set "PYENV_ROOT=%USERPROFILE%\.pyenv\pyenv-win"

REM Specific Python version to use
set "PYENV_PYVER=3.12.10"
set "PYENV_PYDIR=%PYENV_ROOT%\versions\%PYENV_PYVER%"
set "PYENV_PY=%PYENV_PYDIR%\python.exe"

if not exist "%PYENV_PY%" (
    echo [ERROR] pyenv-win Python %PYENV_PYVER% not found at: "%PYENV_PY%"
    echo         Please install/select it with:   pyenv install %PYENV_PYVER%  ^&^&  pyenv global %PYENV_PYVER%
    exit /b 1
)

REM Create a dedicated virtual environment alongside the launcher
set "STL_VENV=%SCRIPT_DIR%\.stl-pyenv-%PYENV_PYVER%"
set "STL_VENV_PY=%STL_VENV%\Scripts\python.exe"
set "STL_VENV_ACT=%STL_VENV%\Scripts\activate.bat"

if not exist "%STL_VENV_PY%" (
    echo [INFO] Creating venv at "%STL_VENV%" using "%PYENV_PY%"
    "%PYENV_PY%" -m venv "%STL_VENV%" || (
...
REM Ensure venv Scripts is first on PATH so "python" resolves to it everywhere below
set "PATH=%STL_VENV%\Scripts;%PATH%"

REM Explicitly clear PYTHONHOME/PYTHONPATH just in case
set "PYTHONHOME="
set "PYTHONPATH="
...
if not exist "%STL_SHIM%" mkdir "%STL_SHIM%"
(
    echo @echo off
    echo echo [PYENV MODE] Skipping 'conda' invocation: %%*
    echo exit /b 0
) > "%STL_SHIM%\conda.bat"
set "PATH=%STL_SHIM%;%PATH%"
